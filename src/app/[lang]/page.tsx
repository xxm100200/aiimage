...
const Home = ({ params }: { params: { lang: string } }) => {
  ...
  const data = regularData.filter(
    (page: RegularPage) => page.slug === "elements_Collapse",
  )[0];
  const { content } = data;

  const resourcesPath = path.join(process.cwd(), 'src', 'content', language.contentDir, 'sections', 'functionDescription.json')
