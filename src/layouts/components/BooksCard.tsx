...
const BooksCard = async ({ data, lang }: { data: Post; lang: string }) => {
  const { read_more } = await getTranslations(lang);
  const { summary_length, books_folder } = config.settings;
  ...
  return (
    <div className="bg-body dark:bg-darkmode-body">
      ...
      <h4 className="mb-3">
        <Link href={slugSelector(lang, `/${books_folder}/${data.slug}`)}>
          {title}
        </Link>
      </h4>
      <ul className="mb-4">
      ...
      <Link
        ...
        href={slugSelector(lang, `/${books_folder}/${data.slug}`)}
      >
        {read_more}
      </Link>
    </div>
